<!--NO PHYSICS (IE DYNAMIC/STATIC OBJECTS), BOXES WOULD BE AUTOMATICALLY ROTATED WHEN USER GRABS THEM WITH HAND
THEREFORE IF USER IS IN VR MODE THEY ARE INSTEAD RE-DIRECTED TO S4_PHYSICSVR -->
  ...
  <script> 
    ...
    document
      ...
      .addEventListener("click", e => {
        //CHECK IF PREVIOUS TASKS HAVE BEEN COMPLETED
        if (localStorage.getItem("tasksCompleted") >= "4") {
        console.log(localStorage.getItem("vrMode"));
        //CHECK IF IN VR MODE, REDIRECT TO S4_PHYSICSVR IF SO
        if (localStorage.getItem("vrMode") == "true") {
          location.href = "s4_physicsvr.php";   
        } else {
        //IF NOT IN VR MODE, SHOW SCENARIO ON THIS PAGE
