<!-- FIX IMPLEMENTED FOR THE ISSUE SHOWN IN O1_CARDNOTVISIBLE_ISSUE.PHP, NOW YOU CAN ONLY SCAN THE CARD WHEN ITS VISIBLE --> 
...
<script>
     localStorage.setItem("tasksCompleted", "0"); //hard coding tasks completed to 0 to support testing
  console.log("Tasks Completed: " + localStorage.getItem("tasksCompleted"));
  if (localStorage.getItem("tasksCompleted") == "0") { 
    localStorage.setItem("cardVisible", "false");  //initialise variable to indicate card not visible (only if havent scanned it already)
  };
  console.log("Card visible? " + localStorage.getItem("cardVisible"));
</script>
    ...
    AFRAME.registerComponent('collide', {
      init: function() {
       ...
       this.el.addEventListener('hit', (e) => {
        //timeout function sourced/adapted from https://stackoverflow.com/questions/68295433/a-frame-trigger-function-when-two-objects-touch
        setTimeout(() => {
  ...
  <body>
    ...
    <a-scene vr-mode-ui-if-headset>
      ...
      <a-box id="task"
       ...
       <!--CARD AND SCANNER - CARD INITIALLY NOT VISIBLE, ANIMATION APPLIED LATER IN CODE  
       LINKED TO COMPONENT TO MAKE SCANNER GREEN AND SPHERE APPEAR WHEN OBJECTS COLLIDE-->
       <a-box id="scanner" sphere-collider="" height="9" width="1.2" depth="1.2" position="12.1 -4.6 -28"  color="black"></a-box>
      <a-box id="card" 
        src="<?php echo "$student_card_asset"?>" 
        position="<?php echo "$student_card_position"?>"
        rotation="<?php echo "$student_card_position"?>"
        width="4" height="2" depth="2" visible="false" look-at="#myCamera"
        collide></a-box> 
  ...
  <script> 
    ...
    //CHECK IF PREVIOUSLY SCANNED STUDENT CARD 
        if (localStorage.getItem("tasksCompleted") >= "1") {
          document.querySelector("#scanner").setAttribute('color', 'green');
          document.querySelector("#g1").setAttribute('visible', 'true');
          document.querySelector("#card").setAttribute('visible', 'true');
          document.querySelector("#card").setAttribute('position', '12.1 -2.1 -28');
        }
      
      
    ///CLICK ON TASK BOX TO DISPLAY STUDENT CARD
      document
      .querySelector("#task")
    ...
    ///CLICK ON CARD TO SCAN IT (INCLUDES ANIMATION AND MARKING TASK AS COMPLETED)
    document
      .querySelector("#card")
        .addEventListener("click", e => {

        //ONLY EXECUTE IF CARD IS VISIBLE
        if (localStorage.getItem("cardVisible") == "true") {

          //APPLY ANIMATION (INCLUDED HERE RATHER THAN AS ANIMATION WITHIN CARD ENTITY AS ONLY WANT TO EXECUTE IF CARD VISIBLE)
          document.querySelector("#card").setAttribute('animation__mousedown','property: position; to: 12.1 -2.1 -28; dur: 2000; easing: linear; loop: false; startEvents: mousedown');
