<!--VARIATION OF LOCATIONS/G1.PHP AND VIDEOS/G1_VIDEO.PHP - SHOWING HOW VIDEO CAN INSTEAD BE EMBEDDED WITHIN THE PAGE -->
...
$location_code = "g1"; //hardcoded as page was renamed when taken out of main tour
    ...
    <a-scene vr-mode-ui-if-headset>
      ...
      <a-assets>
        <video id="linked_video" crossorigin="anonymous"
          ...
          autoplay="true" loop="false" onloadstart="this.playbackRate = 6;" onended="videoEnded()">
        </video>
      </a-assets>
      ...
      <a-sphere id="g3" radius="1" 
        ...
        value="<?php echo "$g3_link_text" ?>" position="0 -1.5 0" ></a-text>
      </a-sphere>
  
      <a-sphere id="g5" radius="1" 
        src="<?php echo "$g5_link_asset" ?>" 
        position="<?php echo "$g5_link_position" ?>"  
        rotation="<?php echo "$g5_link_rotation" ?>" 
        event-set__enter="_event: mouseenter; scale: 1.2 1.2 1.2"
        event-set__leave="_event: mouseleave; scale: 1 1 1">
        <a-text font="kelsonsans" align="center" look-at="#myCamera" width="7" 
        value="<?php echo "$g5_link_text" ?>" position="0 -1.5 0" ></a-text>
      </a-sphere>
      ...
      <!--VIDEOSPHERE  rotation value hard coded so as not to affect database values for the actual final implemented video page-->           
      ...
      <!--EXIT BUILDING-->
      <a-box id="exit" 
        src="<?php echo "$exit_info_asset" ?>" 
        position="<?php echo "$exit_info_position" ?>"  
        rotation="<?php echo "$exit_info_rotation" ?>"
        event-set__enter="_event: mouseenter; _target: #exitText; visible: true"
        event-set__leave="_event: mouseleave; _target: #exitText; visible: false">
        <a-text id="exitText" value="<?php echo "$exit_info_text" ?>" position="0.05 -1.5 -0.05" 
        align="center" color="#FFFFFF" width="7" font="kelsonsans" visible="false" look-at="#myCamera"
        geometry="primitive: plane; height: 0; width: 0" material="color: #333333"></a-text>
      </a-box>
      ...
      <a-box id="task"
        ...
        <a-text id="taskText" value="<?php echo "$task_info_text" ?>" position="0.05 1.5 -0.05" 
        align="center" color="#FFFFFF" width="7" font="kelsonsans" visible="false" look-at="#myCamera"
        geometry="primitive: plane; height: 0; width: 0" material="color: #333333"></a-text>
      </a-box>
    </a-scene>
  ...
  <script> 
    ...
    .querySelector("#task")
      .addEventListener("mouseenter", e => {
        ...
        if (localStorage.getItem("tasksCompleted") >= "2") {
          ...
          localStorage.setItem("tasksCompleted","2");
        }
        console.log("Tasks Completed: " + localStorage.getItem("tasksCompleted"));
    });
    ...
    document
      ...
      .addEventListener("click", e => {
       // location.href = "o1.php";   //link to main tour disabled as this is just a testing page 
    });
    ...
    document
      .querySelector("#g5")
      .addEventListener("click", e => {
        //location.href = "g5.php";   //link to main tour disabled as this is just a testing page 
    });
    ...
    //PLAY VIDEO
     document
      ...
      .addEventListener("click", e => {
        ...
        document.querySelector('#g3').setAttribute('visible', 'false')
        document.querySelector('#g5').setAttribute('visible', 'false')
        document.querySelector('#video').setAttribute('visible', 'false')
        document.querySelector('#exit').setAttribute('visible', 'false')
        document.querySelector('#task').setAttribute('visible', 'false')
        document.querySelector('#videosphere').setAttribute('visible', 'true') 
        let video = document.querySelector("#linked_video");
        video.play();
    });
    ...
    function videoEnded() {
        ...
        document.querySelector('#g3').setAttribute('visible', 'true')
        document.querySelector('#g5').setAttribute('visible', 'true')
        document.querySelector('#video').setAttribute('visible', 'true')
        document.querySelector('#exit').setAttribute('visible', 'true')
        document.querySelector('#task').setAttribute('visible', 'true')
       }
