<!-- THIS PAGE SHOWS THAT BOXES SEEMINGLY WENT THROUGH THE WALL/DOOR WHEN PUSHED AS PART OF THE TASK
THIS WAS BEFORE THE INIVISBLE WALL WAS IMPLEMENTED IN Q1.PHP -->
...
<script>
  localStorage.setItem("tasksCompleted", "6"); //hard coding tasks completed to 6 to support testing
...
$location_code = "q1"; //hardcoded as page was renamed when taken out of main tour
...
<html lang="en">
  ...
  <body>
    ...
    <a-scene vr-mode-ui-if-headset>
      ...
      <a-sphere id="f3" radius="1" 
        ...
        value="<?php echo "$f3_link_text" ?>" position="0 -1.5 0" ></a-text>
      </a-sphere>
      
      <a-sphere id="q2" radius="1" 
        src="<?php echo "$q2_link_asset" ?>" 
        position="<?php echo "$q2_link_position" ?>"  
        rotation="<?php echo "$q2_link_rotation" ?>" 
        event-set__enter="_event: mouseenter; scale: 1.2 1.2 1.2"
        event-set__leave="_event: mouseleave; scale: 1 1 1">
        <a-text font="kelsonsans" align="center" look-at="#myCamera" width="7" 
        value="<?php echo "$q2_link_text" ?>" position="0 -1.4 0" ></a-text>
      </a-sphere>
  
      <a-sphere id="q3" radius="1" 
        src="<?php echo "$q3_link_asset" ?>" 
        position="<?php echo "$q3_link_position" ?>"  
        rotation="<?php echo "$q3_link_rotation" ?>" 
        event-set__enter="_event: mouseenter; scale: 1.2 1.2 1.2"
        event-set__leave="_event: mouseleave; scale: 1 1 1">
        <a-text font="kelsonsans" align="center" look-at="#myCamera" width="7" 
        value="<?php echo "$q3_link_text" ?>" position="0 -1.5 0" ></a-text>
      </a-sphere>
      ...
      <a-box width="75" height="0.01" depth="75" static-body visible="false" position="0 -4 0"></a-box>
       ...
       <!-- Boxes -->
      <a-entity id= "interactive_scenario" visible="false" position="0 0 0">
  ...
  <script> 
    ...
    if (localStorage.getItem("tasksCompleted") >= "5") {
      ...
      document.querySelector('#q2').setAttribute('visible', 'false')      //dont want link to be visible immediately for those completing tasks
    } else {
    ...
    document
      ...
      .addEventListener("click", e => {
        ...
        //SHOW INTERACTIVE SCENARIO (IF PREVIOUS TASKS COMPLETED)
        if (localStorage.getItem("tasksCompleted") >= "5") {
        document.querySelector('#interactive_scenario').setAttribute('visible', 'true')
        document.querySelector('#q2').setAttribute('visible', 'true')
        }
        ...
        //Adapted from: https://stackoverflow.com/questions/27765666/passing-variable-through-javascript-from-one-html-page-to-another-page      
          if (localStorage.getItem("tasksCompleted") >= "6") {
            //dont change 
          } else if (localStorage.getItem("tasksCompleted") == "5") {
            localStorage.setItem("tasksCompleted","6");
          }
          console.log("Tasks Completed: " + localStorage.getItem("tasksCompleted"));
    });
    ...
    document
      .querySelector("#q2")
      .addEventListener("click", e => {
        //location.href = "q2.php"; //link to main tour disabled as this is just a testing page 
    });

    document
      .querySelector("#q3")
      .addEventListener("click", e => {
        //location.href = "q3.php"; //link to main tour disabled as this is just a testing page 
    });

    document
      .querySelector("#f3")
      .addEventListener("click", e => {
       // location.href = "f3.php"; //link to main tour disabled as this is just a testing page 
    });
